...
--#4. Show the team1, team2 and player for every goal 
...
--#6. List the the dates of the matches and the name of 
...
SELECT stadium, COUNT(player) AS goals_scored_each_stadium
FROM game 
JOIN goal
ON matchid = id
GROUP BY stadium;
...
SELECT matchid, mdate, COUNT(player) AS num_of_goals
FROM game 
JOIN goal
ON id = matchid
WHERE teamid = 'GER'
