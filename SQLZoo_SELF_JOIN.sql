...
--       Run the query and notice the two services that link these 
...
-- ## 5. Execute the self join shown and observe that b.stop gives 
...
--       taking one bus, including 'Craiglockhart' itself, 
--       offered by the LRT company. Include the company and bus no. of the relevant services.
SELECT stops.name, b.company, b.num 
FROM route AS a
JOIN route AS b
ON a.num = b.num AND a.company = b.company
JOIN stops
ON stops.id = b.stop
WHERE a.stop = (SELECT id FROM stops WHERE name = 'Craiglockhart') AND a.company = 'LRT';
